 ...
 *
 * Auth is `requireChurchUser` rather than `requireChurchAdmin` — the same
 * gate /api/song-lookup uses. Who may add a photograph is decided in the client
 * by the gallery capability; the job here is to keep everyone outside the
 * church from filling the store.
 *
 * Every file a church stores goes under a folder named for it —
 * uec/gallery/<albumId>/… — and a church may only delete inside its own. The
 * store is shared by every church on the platform, so that prefix is the whole
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "DELETE") {
      ...
      //
      // Only this church's own files: a URL outside its folder is skipped, not
      // refused, so an album holding a photograph from before the move to
      // per-church folders can still be deleted without an error.
      const prefix = `/${caller.church.id}/`;
    ...
    const blob = await put(`${caller.church.id}/${folder}/${randomUUID()}.${extension}`, decoded.buffer, {
      access: "public",

...
//
// Every church on the platform is mailed separately: the scheduled run walks
// the open churches one at a time, and each digest reads, and goes to, that
// church's own calendar and accounts only. A manual send is for the church the
// administrator pressing the button belongs to.
...
//
// A church on the platform has an address of its own, and that comes first of
// all: uec.church.app, or the domain it brought.
const appUrlFor = (church, req) =>
  (
    originOf(church) ||
    process.env.APP_URL ||
...
async function runDigest({ firestore, churchName, kind, today, appUrl, preview, trigger, actor }) {
  const [church, calendar] = await Promise.all([
    loadChurch(firestore, churchName),
    loadCalendar(firestore),
  ]);
...
async function runActivityReport({ firestore, churchName, today, range, appUrl, admin, preview }) {
  const from =
    range === "all"
      ? null
      : range === "quarter"
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      // `auto` depends on the date, and the date depends on each church's
      // timezone, so it is worked out per church below.
      const kinds = DIGEST_KINDS.includes(requested) ? [requested] : [];
      if (requested !== "auto" && !kinds.length) {
        return res.status(400).json({ error: `Unknown digest type: ${requested}` });
      }

      // Church by church, digest by digest. Sequential on purpose — Gmail
      // dislikes parallel SMTP bursts — and one church failing must not cost
      // every church after it its morning email.
      const churches = [];
      for (const church of await listOpenChurches()) {
        const timeZone = church.data.timezone || DEFAULT_TIMEZONE;
        const today = zonedDateString(new Date(), timeZone);
        const results = [];
        try {
          for (const kind of requested === "auto" ? scheduledKinds(today) : kinds) {
            results.push(
              await runDigest({
                firestore: church.ref,
                churchName: church.data.name,
                kind,
                today,
                appUrl: appUrlFor(church, req),
                trigger: "cron",
                preview: false,
              })
            );
          ...
          console.error(`Digest failed for ${church.id}:`, error);
          results.push({ error: error.message || "Failed" });
        }
        churches.push({ church: church.id, today, timeZone, results });
      }
    ...
    const firestore = admin.church.ref;
    const churchName = admin.church.data.name;
    const appUrl = appUrlFor(admin.church, req);
    const today = zonedDateString(new Date(), admin.church.data.timezone || DEFAULT_TIMEZONE);
    ...
    if (type === "activity") {
      ...
      const result = await runActivityReport({ firestore, churchName, today, range, appUrl, admin, preview });

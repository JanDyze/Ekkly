...
import { loadChurch, resolveChurchId } from "../lib/tenant.js";
import {
  collectOccurrences,
  zonedDateString,
  addDays,
 ...
 */
const publicImage = (value, kind, stamp, churchId) => {
  if (!value) return "";
...
const publicChurch = (church = {}, stamp, churchId) => ({
  ...
  logo: publicImage(church.logo, "logo", stamp, churchId),
  logoDark: publicImage(church.logoDark, "logoDark", stamp, churchId),
});
 ...
 * thing to publish.
 */
const publicLanding = (landing = {}, stamp, churchId) => {
  const { heroImage, hiddenAlbums, ...rest } = landing;
  return {
    ...rest,
    heroImage: publicImage(heroImage, "hero", stamp, churchId),
  };
};
...
export default async function handler(req, res) {
  ...
  let church;
  try {
    // The church this page belongs to: `?church=` from the app, or the address
    // it was opened on. Everything below reads inside that church alone.
    const churchId = await resolveChurchId(req);
    church = churchId ? await loadChurch(churchId) : null;
  } catch {
  ...
  try {
    if (!church) return res.status(404).json({ error: "No church here" });
    const firestore = church.ref;
    ...
    return res.status(200).json({
      enabled: true,
      church: publicChurch(data.church, stamp, church.id),
      landing: publicLanding(landing, stamp, church.id),
      gatherings,
      photos,
      timeZone,
    });

...
// The worker is a static file in public/, so it cannot read import.meta.env.
// It is handed the project's (public) web config in its URL instead, which
// keeps one Firebase project named in one place — the env — rather than a
// second copy hardcoded in the worker that goes stale the day the project
// changes.
...
const isFcmRegistration = (reg) =>
  [reg.active, reg.waiting, reg.installing].some((sw) =>
    sw?.scriptURL?.split("?")[0].endsWith("firebase-messaging-sw.js")
  );

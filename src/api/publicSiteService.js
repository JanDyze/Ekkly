...
import { withChurchParam } from './church'
...
export const fetchPublicSite = async () => {
  ...
  inFlight = (async () => {
    // The church goes in the query string rather than a header: this answer is
    // cached at the edge, and the URL is what the cache is keyed on.
    const response = await fetch(withChurchParam('/api/public'), {

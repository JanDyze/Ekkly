...
import {
  providerLabel,
  removeUserAccount,
  syncAccountsFromAuth,
} from '../api/userAccountsService'
import {
  approveJoinRequest,
  declineJoinRequest,
  revokeAccess,
...
/* ---------------------------------------------------------- join requests */
// Signing in no longer lets anybody into the church. Whoever has asked waits
// here for an administrator, and approving them is what opens the app for
// them — on their screen, live, without them refreshing.
const joinRequests = ref([])
const deciding = ref('')
let stopJoinRequests = null

onMounted(() => {
  stopJoinRequests = subscribeToJoinRequests((list) => {
    joinRequests.value = list
  })
})
onUnmounted(() => stopJoinRequests?.())

const approve = async (request) => {
  deciding.value = request.uid
  try {
    await approveJoinRequest(request, user.value)
    toast.success(`${request.displayName || request.email} can now open the app`)
  } catch (error) {
    console.error('Error approving join request:', error)
    toast.error(
      error?.code === 'permission-denied'
        ? 'Only administrators can let people in.'
...
const decline = async (request) => {
  deciding.value = request.uid
  try {
    await declineJoinRequest(request, user.value)
    toast.success('Request declined')
  } catch (error) {
    console.error('Error declining join request:', error)
    toast.error(
      error?.code === 'permission-denied'
        ? 'Only administrators can answer requests.'
...
// Taking somebody out of the church: their access, any administrator role,
// and their row on this page. Their Google account and member record stay.
const removing = ref(false)

const removeFromChurch = async (account) => {
  if (!window.confirm(`Remove ${account.name} from the church? They will need to ask to join again.`)) return
  removing.value = true
  try {
    await revokeAccess(account.uid)
    await removeUserAccount(account.uid).catch(() => {})
    selected.value = null
    ...
    toast.error(
      error?.code === 'permission-denied'
        ? 'Only administrators can remove people.'
...
<template>
  <div class="h-full overflow-y-auto pb-bar!">
    ...
    <div class="mb-4 flex items-start justify-between gap-3">
      <div class="min-w-0">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          Everyone who can open this church's app, and when they were last here
        </p>
      </div>
      <button
        @click="handleSync"
    ...
    <!-- Asking to join: first, because somebody is waiting on it. -->
    <section v-if="joinRequests.length" class="mb-5">
      <h2 class="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">
        <UserPlus class="h-4 w-4" />
        Asking to join &middot; {{ joinRequests.length }}
      </h2>
      <ul class="space-y-2">
        <li
          v-for="request in joinRequests"
          :key="request.uid"
          class="rounded-xl border border-amber-200 dark:border-amber-500/30 bg-amber-50/60 dark:bg-amber-500/5 p-3"
        >
          <div class="flex items-center gap-3">
            ...
            <div
              v-else
              class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-200 dark:bg-gray-700"
            >
              <UserRound class="h-5 w-5 text-gray-400" />
            </div>
            <div class="min-w-0 flex-1">
              <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">
                {{ request.displayName || request.email || 'Unnamed account' }}
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                {{ request.email }}<template v-if="request.requestedAt"> &middot; {{ timeAgo(request.requestedAt) }}</template>
              </p>
            </div>
          </div>
          <p
          ...
          <div class="mt-3 flex gap-2">
            <button
              ...
              class="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg bg-primary text-sm font-semibold text-white disabled:opacity-60"
              @click="approve(request)"
            >
              <Loader2 v-if="deciding === request.uid" class="h-4 w-4 animate-spin" />
              <Check v-else class="h-4 w-4" />
              Let in
            </button>
            <button
              :disabled="deciding === request.uid"
              class="flex h-9 items-center justify-center gap-1.5 rounded-lg border border-gray-200 dark:border-gray-700 px-3 text-sm font-semibold text-gray-600 dark:text-gray-300 disabled:opacity-60"
        ...
        >
          ...
          >
            ...
            <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <!-- Out of the church -->
              <button
                v-if="!isMe(selectedAccount)"
                :disabled="removing"
                class="w-full flex items-center justify-center gap-2 h-11 rounded-xl border border-red-200 dark:border-red-500/30 text-sm font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-60"
                @click="removeFromChurch(selectedAccount)"
              >

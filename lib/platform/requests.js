...
export async function approve(admin, { requestId, churchId, churchName }) {
  ...
  const name = await firestore.runTransaction(async (tx) => {
    ...
    //
    // `setup.done: false` is what sends its first administrator to the setup
    // guide instead of into the app. Written here rather than worked out from
    // the settings, because "has this church been set up?" is a fact about the
    // church and not a guess about its content — and a church from before the
    // guide existed has no `setup` field at all, which is how it stays out of
    // their way.
    //
    // Nothing about the public page being on or off: it is on from the first
    // day, wearing the starting words in src/data/appDefaults.js until the
    // guide or Settings > Public page replaces them.

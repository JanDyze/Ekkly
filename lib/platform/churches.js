...
export function planOf(appsData, billingData, catalog) {
  ...
  return {
    ...
    // Whether a card pays for this church, without PayMongo's ids.
    card: {
      on: Boolean(billingData?.card?.subscriptionId) && !["cancelled", "incomplete_cancelled"].includes(billingData?.card?.status),
      status: billingData?.card?.status || "",
      cycle: billingData?.card?.cycle || "",
      amount: billingData?.card?.amount || 0,
      nextBilling: billingData?.card?.nextBilling || "",
    },
  };
}

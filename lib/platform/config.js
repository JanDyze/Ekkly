...
const commit = async (admin, ref, data, action, label, cacheKey) => {
  ...
  // Each field is replaced whole, and the rest of the document kept. A plain
  // merge would merge maps key by key, so a colour that was cleared — or all of
  // them, on "Back to the original colours" — would stay underneath.
  const fields = [...Object.keys(data), "updatedAt"];
  batch.set(ref, { ...data, updatedAt: FieldValue.serverTimestamp() }, { mergeFields: fields });

...
import { repriceCardBilling } from "./payments.js";
...
export async function setChurchApps(admin, { churchId, apps, lockedOff = [] }) {
  ...
  await batch.commit();
  // A card on file is charged the new total from its next cycle.
  await repriceCardBilling(churchId, church.get("name"));
...
export async function setMyApps(caller, { apps }) {
  ...
  await batch.commit();
  await repriceCardBilling(churchId, caller.church.data?.name);

...
import { DEFAULT_THEME, isHexColor } from "./platformDefaults.js";

/* ------------------------------------------------------------------ theme */

// The church's accent, the same colour its app wears (its own, the platform's,
// or Ekkly's blue). Set at the start of each build from `church.accent`; the
// builders are synchronous, so one email's colour cannot leak into another's.
let BRAND = DEFAULT_THEME.primary;
const useAccent = (church) => {
  BRAND = isHexColor(church?.accent) ? church.accent : DEFAULT_THEME.primary;
};
const INK = "#111827";
const MUTED = "#6b7280";
const FAINT = "#9ca3af";

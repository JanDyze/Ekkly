...
export const brandFont = (key) => BRAND_FONTS.find((f) => f.key === key) || BRAND_FONTS[0]
...
export const fontsUrl = (keys) => {
  const families = [...new Set(keys)]
    ...
    .map((font) => `family=${font.family.replace(/ /g, '+')}:${font.spec}`)
  return families.length ? `https://fonts.googleapis.com/css2?${families.join('&')}&display=swap` : ''
}

/** Only valid colours and a known face survive; anything else is left out, so it falls back. */
...
export const resolveTheme = (platformTheme, churchTheme) => {
  ...
  return {
    ...
    font: church.font || platform.font || DEFAULT_THEME.font,
  }
}

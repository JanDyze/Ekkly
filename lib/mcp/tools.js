...
/**
 * The tools this deployment actually offers, given its write setting and the
 * church's apps (a Set of app keys, or null for every app).
 */
export const enabledTools = (allowWrites, apps = null) =>
  TOOLS.filter((tool) => (allowWrites || !tool.write) && (!apps || !TOOL_APPS[tool.name] || apps.has(TOOL_APPS[tool.name])));

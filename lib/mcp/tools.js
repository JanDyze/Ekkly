...
//   address attached.
// Every read and write goes through church(): the church this request’s token
// belongs to, and nothing outside it (lib/mcp/church.js).
...
// The schedule's roles and how a service's assignments are read, shared with
// the Schedules page for the same reason: an old lineup has to read as a
// schedule here exactly as it does there.
import { assignmentsOf, scheduleRolesFrom } from "../../src/data/scheduleRoles.js";
import {
  ageOn,
  asDate,
  cached,
...
const churchProfile = async () => {
  ...
  // Not `church`: that is the imported reader this function calls above, and
  // shadowing it here put the call in its own temporal dead zone.
  const profile = data.church || {};
  const landing = data.landing || {};

  // What the church says about itself moved out of the landing block and onto
  // the church itself in v0.29.4, so each of these reads the new home first and
  // the old one while a church has not edited it — the same fallback the app
  // and the public page use (withChurchDefaults in src/data/appDefaults.js).
  const said = (key) => profile[key] || landing[key] || "";

  // The church's pair lists — an article and its scripture, a value and what it
  // means — with whatever was left half-written dropped.
  const pairs = (list, first, second) =>
    (Array.isArray(list) ? list : [])
      .filter((entry) => String(entry?.[first] || "").trim())
      .map((entry) => ({
        [first]: String(entry[first]).trim(),
        [second]: String(entry[second] || "").trim(),
      }));

  return {
    name: profile.fullName || profile.shortName || "",
    ...
    basisOfFaith: pairs(profile.basisOfFaith, "belief", "reference"),
    // What the church holds to, as the phrase and the sentence under it.
    values: pairs(profile.values, "value", "note"),
    // Still the landing block's: the "who we are" paragraphs are what the
    // public page says about the church, and they are written with that page.
    about: landing.about || "",
    address: said("address"),
    phone: said("phone"),
    email: said("email"),
    facebook: said("facebook"),
...
const getMember = async (args = {}) => {
  ...
  const doc = await church().collection("members").doc(member.docId).get();
  const relatives = doc.exists ? doc.data().relatives || {} : {};

  const [groups, tasks] = await Promise.all([
    church().collection("smallGroups").get(),
    church().collection("tasks").where("done", "==", false).get(),
  ]);
...
const searchSongs = async (args = {}) => {
  ...
  const [songs, members] = await Promise.all([
    cached("songs", () => church().collection("worshipSongs").get().then(docsOf)),
    loadMembers(),
  ]);
...
const getLineup = async (args = {}) => {
  ...
  // Still the worshipLineups collection: the page is called Schedules now, but
  // the months already planned were never moved.
  const [doc, members, settings] = await Promise.all([
    church().collection("worshipLineups").doc(month).get(),
    loadMembers(),
    church().collection("appSettings").doc("church").get(),
  ]);
  if (!doc.exists) {
    return { month, planned: false, message: "Nothing has been scheduled for that month." };
  }

  const index = indexMembers(members);
  const data = doc.data();
  const roles = scheduleRolesFrom((settings.exists ? settings.data() : {}).scheduleRoles);

  /**
   * Everyone on a Sunday, role by role, in the order the church lists roles.
   * A role since removed from the list still has people on file; they are
   * reported under its stored id rather than silently left out, because "who
   * was on the door that week" is a fair question about an old schedule.
   */
  const rolesOn = (sunday) => {
    const assignments = assignmentsOf(sunday);
    const known = roles
      .filter((role) => assignments[role.id]?.length)
      .map((role) => ({
        role: role.name,
        people: assignments[role.id].map((id) => nameOf(index, id)),
      }));
    const removed = Object.keys(assignments)
      .filter((id) => !roles.some((role) => role.id === id))
      .map((id) => ({
        role: `${id} (role no longer listed)`,
        people: assignments[id].map((personId) => nameOf(index, personId)),
      }));
  ...
  return {
    ...
    sundays: (Array.isArray(data.sundays) ? data.sundays : [])
      ...
      .map((sunday) => ({
        ...
        // Every role, the song leader and band included, so a question about
        // ushers or the preacher is answered from the same list.
        roles: rolesOn(sunday),
...
const listPrayerConcerns = async (args = {}) => {
  ...
  const concerns = await church().collection("prayerConcerns").get().then(docsOf);
...
const listSmallGroups = async (args = {}) => {
  ...
  const [groups, members] = await Promise.all([
    church().collection("smallGroups").get().then(docsOf),
    loadMembers(),
  ]);
  const index = indexMembers(members);
...
const searchMinutes = async (args = {}) => {
  ...
  const minutes = await church().collection("minutes").get().then(docsOf);
...
const getMinute = async (args = {}) => {
  const doc = await church().collection("minutes").doc(String(args.minuteId || "")).get();
...
const listTasks = async (args = {}) => {
  const limit = clampLimit(args.limit, 40, 200);
  const tasks = await church().collection("tasks").get().then(docsOf);
  const now = today();
...
const financeSummary = async (args = {}) => {
  ...
  const entries = (await church().collection("ledgerEntries").get().then(docsOf)).filter(
 ...
 *
 *   Every write says where it came from. createdBy / updatedBy stay MCP_ACTOR_ID
 *   whoever connected, and the matching Name fields carry the owner of the
 *   link (lib/mcp/church.js actorName), so a record changed through a
 *   conversation can be told from one somebody typed — and from whose link.
 *
...
const addMember = async (args = {}) => {
  ...
  const ref = await church().collection("members").add(record);
...
const updateMember = async (args = {}) => {
  ...
  await church().collection("members").doc(member.docId).update(changes);
  invalidate("members", "calendar");
...
const updateEvent = async (args = {}) => {
  ...
  const vocabulary = await loadVocabulary();
  const events = church().collection("events");
...
const updatePrayerConcern = async (args = {}) => {
  ...
  const doc = await church().collection("prayerConcerns").doc(id).get();
...
const updateTask = async (args = {}) => {
  ...
  if (args.done !== undefined) {
    ...
    edits.doneByName = edits.done ? actorName() : "";
  }
  if (!Object.keys(edits).length) throw new Error("Nothing to change: no fields were given.");

  const doc = await church().collection("tasks").doc(id).get();
...
const updateSmallGroupMembers = async (args = {}) => {
  ...
  const doc = await church().collection("smallGroups").doc(groupId).get();
...
const addLedgerEntry = async (args = {}) => {
  ...
  const ref = await church().collection("ledgerEntries").add(entry);
...
export const TOOLS = [
  {
    ...
    description:
      "The church's own details: name, branch, when it was founded and what it is affiliated " +
      "with, its mission, vision, basis of faith and core values, address and contacts, the " +
  ...
  {
    name: "get_lineup",
    title: "A month's schedule",
    description:
      "The schedule for one month (the app used to call it the lineup): for each Sunday, the " +
      "theme, everyone serving by role — song leader, band, preacher, ushers, Sunday school " +
      "teachers and whatever other roles the church lists — and the songs in order with the key " +
      "each will be sung in. `leader` and `team` repeat the worship roles for older callers. " +
      "Defaults to the current month. A month still marked draft is not settled.",
...
/**
 * Which app each tool belongs to (lib/apps.js). A church that has switched an
 * app off has its tools left out of the connector too: answering questions
 * about a ledger the church no longer keeps in the app would be the one place
 * the app went on quietly doing a thing it had been told to stop. Tools not
 * listed — the church profile, the roll — belong to no app that can go.
 */
const TOOL_APPS = {
  list_events: "events",
  create_event: "events",
  ...
  add_song: "songs",
  get_lineup: "lineups",
  list_prayer_concerns: "prayer",
  add_prayer_concern: "prayer",
  update_prayer_concern: "prayer",
  list_small_groups: "smallgroups",
  get_small_group: "smallgroups",
  update_small_group_members: "smallgroups",
  ...
  add_ledger_entry: "finances",
};

/**
 * Which capability area each tool answers to. An area is an app key
 * (lib/capabilities.js, lib/apps.js), so TOOL_APPS is most of the map already;
 * the roll belongs to no app that can be switched off but is very much behind
 * a capability, so it is added here. `church_profile` is in neither: the
 * church's own name, timezone and ministries are what any signed-in account
...
/**
 * Whether the person whose link this is could do this in the app: `.view` to
 * read, `.manage` to change. A link can never do more than its owner can, so
 * the write switch on the link is a ceiling and not a grant.
 *
 * `capabilities` is null for the MCP_TOKEN environment door, which belongs to
 * the deployment rather than to a person and so has nobody to ask about.
 */
const permitted = (tool, capabilities) => {
  if (!capabilities) return true;
  const area = TOOL_AREAS[tool.name];
  if (!area) return true;
  return capabilities.has(`${area}.${tool.write ? "manage" : "view"}`);
};

/**
 * The tools this connector actually offers: the ones its link allows to write,
 * that belong to an app the church still has, and that the link's owner could
 * use in the app themselves.
 *
 * @param apps          a Set of the church's app keys, or null for every app
 * @param capabilities  a Set of the owner's capabilities, or null for no owner
 */
export const enabledTools = (allowWrites, apps = null, capabilities = null) =>
  TOOLS.filter(
    (tool) =>
      (allowWrites || !tool.write) &&
      (!apps || !TOOL_APPS[tool.name] || apps.has(TOOL_APPS[tool.name])) &&

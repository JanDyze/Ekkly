...
import { MCP_ACTOR, MCP_ACTOR_ID } from "./data.js";
 ...
 */
export async function respond(message, { allowWrites = false, apps = null } = {}) {
  ...
  try {
    switch (method) {
      ...
      case "tools/list":
        return result(id, { tools: enabledTools(allowWrites, apps).map(advertise) });

      case "tools/call": {
        const tool = enabledTools(allowWrites, apps).find((t) => t.name === params.name);
        if (!tool) {
        ...
        if (tool.write) {
          try {
            await auditToolWrite({
              ...
              args,
              result: answer,
              actor: { uid: MCP_ACTOR_ID, name: MCP_ACTOR },
            });
          } catch (error) {

...
import { enabledTools, TOOLS } from "./tools.js";
import { auditToolWrite } from "../audit.js";
import { currentActor, MCP_ACTOR_ID } from "./church.js";
 ...
 * @param options.apps the church's apps (Set), or null for every app
 * @param options.capabilities what the link's owner may do, or null for no owner
 * @returns the response to send, or null when nothing should be sent
 */
export async function respond(message, { allowWrites = false, apps = null, capabilities = null } = {}) {
  ...
  try {
    switch (method) {
      ...
      case "tools/list":
        return result(id, { tools: enabledTools(allowWrites, apps, capabilities).map(advertise) });

      case "tools/call": {
        const tool = enabledTools(allowWrites, apps, capabilities).find((t) => t.name === params.name);
        if (!tool) {
          // A tool that exists but is switched off deserves to say so, rather
          // than looking to the model like a name it hallucinated — and the
          // three reasons are worth telling apart, because the model can act
          // on the difference: the link is read-only, the person whose link it
          // is cannot do this in the app either, or the name was invented.
          const real = TOOLS.find((t) => t.name === params.name);
          const readOnly = Boolean(real?.write) && !allowWrites;
          return failure(
            id,
            ...
            readOnly
              ...
              : real
                ? `Unknown tool "${params.name}". It is not available on this connector — the church may not have that app, or the person whose link this is does not have access to it.`
        ...
        let answer;
        try {
          answer = await tool.run(args);
        } catch (error) {
          return result(id, toolFailure(error));
        }

        // A change made through a conversation goes in the same log as one
        // made in the app (lib/audit.js). A record that changed with no entry
        // is worse than an error the model can see, so a failed entry is said.
        if (tool.write) {
          try {
            // The log names the person whose link it was, so it can be read
            // by person; `source: "mcp"` is what still marks it as a change
            // made in a conversation rather than typed.
            const actor = currentActor();
            await auditToolWrite({
              tool: tool.name,
              args,
              result: answer,
              actor: { uid: actor.uid || MCP_ACTOR_ID, name: actor.name },
            });
          } catch (error) {
            console.error("Audit entry for an MCP write failed:", error);
            answer = {
              ...answer,
              warning: "The change was saved, but writing it to the audit log failed.",
            };
          }
        }

        return result(id, toolResult(answer));
      }

...
import { church, currentChurchId } from "./church.js";
import { zonedDateString, DEFAULT_TIMEZONE } from "../occurrences.js";
// Plain data with no browser dependency, imported rather than copied: a second
// chart of accounts or a second list of event types would drift the first time
// somebody edited one of them, and a write validated against a stale copy is a
// write the app itself cannot print.
import { DEFAULT_CATEGORIES } from "../../src/data/appDefaults.js";
...
//
// Keyed by church as well as by name. A warm instance answers every church’s
// connector, and one church’s roll served from the cache to another church’s
// conversation would be the worst thing this file could do.
const keyOf = (key) => `${currentChurchId()}:${key}`;

export async function cached(key, load) {
  const scoped = keyOf(key);
  const hit = cache.get(scoped);
  if (hit && Date.now() - hit.at < TTL_MS) return hit.value;
  const value = await load();
  cache.set(scoped, { at: Date.now(), value });
  return value;
}

/**
 * Drops what a write has just made untrue.
 *
 * Without this the cache above turns into a liar the moment anything writes:
 * add a member and ask who is on the roll in the same breath, and the answer
 * is the list from before the addition, for up to half a minute, with nothing
 * to suggest it is stale. Every write tool calls this for what it touched.
 */
export const invalidate = (...keys) => {
  for (const key of keys) cache.delete(keyOf(key));
};

// Who a record says made the change now lives with the rest of the request's
// context, in church.js, because it varies with the connector's owner. Still
// re-exported from here, where every tool already looks for it.
export { MCP_ACTOR, MCP_ACTOR_ID, actorName } from "./church.js";

/* ----------------------------------------------------------- vocabulary */

/**
 * The controlled lists a write has to respect.
 *
 * A ministry is strict — the app refuses a name that is not in the collection,
 * on the grounds that a ministry is the one thing that grants access, and this
 * has to refuse it too or the connector becomes the way round that rule. Tags
 * are deliberately free text and are not policed here either.
 */
export const loadVocabulary = () =>
  cached("vocabulary", async () => {
    const [ministries, tags, settings] = await Promise.all([
      church().collection("ministries").select("name").get(),
      church().collection("memberTags").select("name").get(),
      church().collection("appSettings").doc("church").get(),
    ]);
    const categories = (settings.exists ? settings.data() : {}).categories || {};
    const names = (snapshot) =>
      snapshot.docs.map((d) => d.data().name).filter(Boolean).sort();

    // A church that has never edited the lists has no `categories` on its
    // settings document at all, and the app falls back to the built-in ones
    // rather than showing an empty picker. Read from the same defaults, or a
    // write here would reject every event type the app itself offers.
    const listOr = (value, fallback) =>
      Array.isArray(value) && value.length ? value : fallback;

    return {
      ministries: names(ministries),
      tags: names(tags),
      eventTypes: listOr(categories.eventTypes, DEFAULT_CATEGORIES.eventTypes),
      songCategories: listOr(categories.songs, DEFAULT_CATEGORIES.songs),
    };
  });

/** Case-insensitive membership, returning the list's own spelling. */
export const canonical = (value, list) => {
  const wanted = String(value ?? "").trim().toLowerCase();
  return list.find((item) => String(item).toLowerCase() === wanted) || null;
};
...
export const loadMembers = () =>
  cached("members", async () => {
    const snapshot = await church().collection("members").select(...MEMBER_FIELDS).get();

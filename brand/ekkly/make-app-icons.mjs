...
const ICONS = {
  // Three people: the middle one, then the two behind step out either side.
  members: svg(`
  <g class="a-side a-left">${person(14, 22, 6.5, 22, 50, 'url(#b)')}</g>
  <g class="a-side a-right">${person(50, 22, 6.5, 22, 50, 'url(#b)')}</g>
  <g class="a-pop">
  ...
  // A calendar: the days fill in, and the last one is checked off.
  attendance: svg(`
  ${splitTile(5, 11, 54, 47, 10, 'c')}
  <rect x="10.5" y="24" width="43" height="28.5" rx="4.5" fill="#fff"/>
  ${[0, 1, 2].map((c) => [0, 1].map((r) => (c === 2 && r === 1 ? '' : `<rect class="a-cell" style="--i:${r * 3 + c}" x="${15 + c * 12}" y="${28 + r * 11}" width="9.5" height="8.5" rx="2" fill="url(#b)"/>`)).join('')).join('')}
  <rect class="a-day" x="39" y="39" width="9.5" height="8.5" rx="2" fill="url(#o)"/>
  ${check(40.8, 39.6, 6, '#fff', 2, 4)}
  ...
  // Coins stacking up, and the peso landing in front.
  finances: svg(`
